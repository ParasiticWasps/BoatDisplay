...
#define VOLUMETRIC_FOG_INCLUDED
 ...
 half3  _SunColor;
    ...
    half _FogScatteringBrightnessThreshold;
...
half3 getShaft(float2 uv) {
    ...
    for (float i = _FogScatteringData.y; i > 0; i--) {
       uv += duv;  
       half4 rgba = SAMPLE_RAW_DEPTH_TEXTURE_LOD(_MainTex, float4(uv.xy,0,0));
       half3 col = max(0.0.xxx, rgba.rgb);
       half brightness = dot(col, half3(0.299, 0.587, 0.114));
       half contrib = step(_FogScatteringBrightnessThreshold, brightness);
       acum += col * (illumination * _FogScatteringData.w * contrib);
...
//           dir.w += frac(dither); // extra banding reduction (not really needed since jitter was introduced but just in case it's needed... here it's)
     ...
     #if FOG_OF_WAR_ON
         if (!f1) sum.a = 1; // ensure fully invisible in fog of war
     #endif
